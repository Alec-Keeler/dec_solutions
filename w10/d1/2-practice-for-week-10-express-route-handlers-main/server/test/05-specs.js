...
describe("05 - GET /artists/:artistId", () => {
  // Reset all the data:
  before(() => {
  ...
  describe("GET /artists/1", () => {
    it("returns the artist with an artistId of 1", async () => {
      await chai
        .request(server)
        .get(`/artists/${1}`)
        .then(res => {
          ...
          expect(res.body).to.eql({
            ...
            albums: [ { albumId: 1, name: 'Stadium Arcadium', artistId: 1 } ]
          });
        });
    });
  });
